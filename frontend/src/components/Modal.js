...
export default function Modal({show, textClose, onClose, textAction ,onAction, header, children}){
    ...
    const modalContent = show ? (
        <div className="flex items-center justify-center fixed z-10 inset-0 overflow-y-auto bg-black bg-opacity-90" >
            <div className="flex rounded-xl items-center justify-center bg-white text-center sm:p-6 sm:pb-4 md_c:items-end">
                ...
                <div>
                    ...
                    {onAction ? <button className='rounded-xl bg-romantic-1 text-brow_pod-1 font-luck text-xl px-8 py-2 hover:bg-orange-500 hover:text-white duration-500' onClick={handleAction}>{textAction ? textAction : "Confirmar"}</button> : null}
                </div>
            </div>
        </div>
    ) : null;
